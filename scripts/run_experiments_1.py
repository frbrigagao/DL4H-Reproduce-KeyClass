...
DATASETS = ['imdb', 'amazon', 'dbpedia', 'agnews']
            ...
            for batch_size in BATCH_SIZES:
                for label_model in LABEL_MODELS:
                    ...
                    if experiment_info:
                        ...
                        print(f"Experiment failed or could not find results folder.")
...
def run_wandb_sweep(keep_configs=False):
    """Run experiments as a W&B sweep"""
    try:
        import wandb
    except ImportError:
        print("wandb not installed. Install it first to use this feature.")
        return
    ...
    sweep_config = {
        ...
        'parameters': {
            ...
            'label_model': {'values': LABEL_MODELS}
        }
    }
    ...
    def sweep_train():
        ...
        label_model = wandb.config.label_model
        
        # Run experiment
        experiment_info = run_experiment(dataset, learning_rate, batch_size, label_model, True, keep_configs)
        
        if experiment_info:
            # Extract metrics
            metrics = extract_metrics(experiment_info['results_folder'])
            
            # Log metrics to wandb including std values
            wandb.log({
                'label_model_accuracy': metrics.get('label_model_accuracy', None),
                'end_model_accuracy': metrics.get('end_model_accuracy', None),
                'end_model_accuracy_std': metrics.get('end_model_accuracy_std', None),
    ...
    # Initialize sweep
    sweep_id = wandb.sweep(sweep_config, project="keyclass-experiments")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--sweep', action='store_true', default=False, help='Run a W&B hyperparameter sweep')
    ...
    if args.sweep:
        print(f"Running W&B sweep with keep_configs={args.keep_configs}")
        run_wandb_sweep(keep_configs=args.keep_configs)
    else:
