...
# notes <- tbl_df(fread('data/NOTEEVENTS.csv', colClasses = c("HADM_ID" = "integer")))

notes <- fread('data/NOTEEVENTS.csv', na.strings = c("", "NA")) %>%
  as_tibble() %>%  # Convert to tibble
  select(ROW_ID, SUBJECT_ID, HADM_ID, CHARTDATE, CHARTTIME, CATEGORY, DESCRIPTION, TEXT) %>% # Keep only these columns
  mutate(
    HADM_ID = as.integer(HADM_ID),
    CHARTTIME = if_else(CHARTTIME == "", NA_character_, CHARTTIME), #handle empty chart time.
  )
...
diagnoses <- tbl_df(fread('data/DIAGNOSES_ICD.csv', colClasses = c("HADM_ID" = "integer")))
...
diagnoses <- diagnoses %>%
  ...
  summarise(ICD9_CODE = paste(ICD9_CODE, collapse = "-"), .groups = "drop")  # Modern way to ungroup
...
notes <- notes %>%
  ...
  group_by(HADM_ID) %>%
  filter(CHARTDATE == min(CHARTDATE)) %>%  # Select earliest CHARTDATE
  filter(!duplicated(HADM_ID))  # Remove duplicates (same day)
  ...
  for (icd9 in icd9row){
    ...
    else
    {
      if (substr(icd9, 1, 1) == "E"){
        icdLevel2 = substr(icd9, 1, 4)
        icd9Top = "cat:18"
      }
      else if (substr(icd9, 1,1) == "V"){
        icdLevel2 = substr(icd9, 1, 3)
        icd9Top = "cat:19"
      }
      else {
    ...
    icd9ListLevel2 = c(icd9ListLevel2, icdLevel2)
    }
